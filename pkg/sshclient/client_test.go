...
func TestNewClientArgs(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "proxy", given: ClientOpts{Proxy: "192.0.2.1"},
			expected: []string{"-A", "-t", "-l", "", "192.0.2.1", "ssh", "-l", "", "-A", "-t", ""}},
		{name: "config", given: ClientOpts{Config: "ssh.config"}, expected: []string{"-F", "ssh.config", "-A", "-t", ""}},
		{name: "user", given: ClientOpts{User: "root"}, expected: []string{"-l", "root", "-A", "-t", ""}},
		{name: "shh options", given: ClientOpts{SSHOptions: []string{"-6", "-C", "-q"}},
			expected: []string{"-o", "-6", "-o", "-C", "-o", "-q", "-A", "-t", ""}},
		{name: "user and proxy", given: ClientOpts{User: "root", Proxy: "192.0.2.1"},
			expected: []string{"-A", "-t", "-l", "root", "192.0.2.1", "ssh", "-l", "root", "-A", "-t", ""}},
		{name: "config and proxy", given: ClientOpts{Config: "ssh.config", Proxy: "192.0.2.1"},
			expected: []string{"-F", "ssh.config", "-A", "-t", "192.0.2.1", "ssh", "-A", "-t", ""}},
		{name: "user and config and proxy", given: ClientOpts{User: "root", Config: "ssh.config", Proxy: "192.0.2.1"},
			expected: []string{"-F", "ssh.config", "-A", "-t", "192.0.2.1", "ssh", "-A", "-t", ""}},
		{name: "options and proxy", given: ClientOpts{SSHOptions: []string{"-6", "-C", "-q"}, Proxy: "192.0.2.1"},
			expected: []string{"-o", "-6", "-o", "-C", "-o", "-q", "-A", "-t", "-l", "", "192.0.2.1",
				"ssh", "-o", "-6", "-o", "-C", "-o", "-q", "-l", "", "-A", "-t", ""}},
	}
...
func TestPrepareCommandEmptyCommand(t *testing.T) {
	...
	assert.Equal(t, []string{"/usr/bin/ssh", "-o", "-6", "-o", "-C", "-o", "-q", "-F", "ssh.config", "-A", "-t", "192.0.2.1",
		"ssh", "-o", "-6", "-o", "-C", "-o", "-q", "-A", "-t", "192.0.2.2"}, cmd.Args)
	cmd = c.prepareCommand([]string{"/bin/bash", "'-c'", "echo \"OK\""})
	assert.Equal(t, []string{"/usr/bin/ssh", "-o", "-6", "-o", "-C", "-o", "-q", "-F", "ssh.config", "-A", "-t", "192.0.2.1",
		"ssh", "-o", "-6", "-o", "-C", "-o", "-q", "-A", "-t", "192.0.2.2", "'", "/bin/bash", "'-c'", "echo \"OK\"", "'"}, cmd.Args)
	assert.Equal(t, os.Stdin, cmd.Stdin)
	assert.Equal(t, os.Stdout, cmd.Stdout)
	assert.Equal(t, os.Stderr, cmd.Stderr)
	assert.Contains(t, buf.String(), "Running: [/usr/bin/ssh -o -6 -o -C -o -q -F ssh.config -A -t 192.0.2.1 "+
		"ssh -o -6 -o -C -o -q -A -t 192.0.2.2]")
}
