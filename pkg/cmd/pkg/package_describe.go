...
func newCmdPackageDescribe(ctx api.Context) *cobra.Command {
	...
	cmd.Flags().BoolVar(&opts.config, "opts.config", false, "Print the opts.configurable properties of the `marathon.json` file")
	cmd.Flags().StringVar(&opts.optionsPath, "describeOptions", "", "Path to a JSON file that contains customized package installation describeOptions")
	cmd.Flags().StringVar(&opts.version, "package-opts.version", "", "The package opts.version")
	cmd.Flags().BoolVar(&opts.allVersions, "package-versions", false, " Displays all versions for this package")
	cmd.Flags().BoolVar(&opts.render, "opts.render", false,
